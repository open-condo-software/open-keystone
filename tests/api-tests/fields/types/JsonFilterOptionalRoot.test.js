...
const optionalRootValueFilterTests = [
  ...
  },
  {
    ...
    case: 'number_gte must be a finite number',
  ...
  },
  {
    ...
    expect_ids: [],
    expected_error: '_gte must be a finite number',
  },
  {
  ...
  },
  {
    ...
    case: 'metadata_match in empty array',
    result: 'in:[] is nothing to match.',
    where: { metadata_match: { in: [] } },
    expect_ids: [],
  },
  {
    id: 'json_error_005_in_empty_array_with_path',
    case: 'metadata_match in with empty array',
    result: 'in:[] is nothing to match.',
    where: { metadata_match: { in: [], path: ['nothing'] } },
    expect_ids: [],
  },
  {
    id: 'json_error_006_not_in_empty_array',
    case: 'metadata_match not_in with empty array',
    result: 'not_in:[] is everything to match.',
    where: { metadata_match: { not_in: [] } },
    expect_ids: ['r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8'],
  },
  {
    id: 'json_error_006_not_in_empty_array_with_path',
    case: 'metadata_match not_in with empty array',
    result: 'not_in:[] is everything to match.',
    where: { metadata_match: { not_in: [], path: ['nothing'] } },
    expect_ids: ['r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8'],
  },
  {
    id: 'json_error_007_whole_field_in_empty_array',
    case: 'metadata_in empty array',
    result: 'metadata_in:[] is nothing to match.',
    where: { metadata_in: [] },
    expect_ids: [],
  },
  {
    id: 'json_error_008_whole_field_not_in_empty_array',
    case: 'metadata_not_in empty array',
    result: 'metadata_not_in:[] is everything to match.',
  ...
  },
  {
    ...
    expect_ids: [],
    expected_error: 'Invalid JSON object key "$ne"',
  },
  {
  ...
  },
  {
    ...
    expect_ids: ['r1', 'r2', 'r3', 'r4', 'r5', 'r6', 'r7', 'r8'],
    expected_error: 'Invalid JSON object key "$ne" for User.metadata',
  },
  {
    id: 'json_security_001_zero_width_path_segment',
    case: 'zero-width joiner inside path segment is rejected',
    result: 'Invisible unicode characters must not pass path validation.',
    where: { metadata_match: { path: ['pro\u200Dfile'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_002_bidi_override_path_segment',
    case: 'bidi override inside path segment is rejected',
    result: 'Bidi control characters must not pass path validation.',
    where: { metadata_match: { path: ['profile\u202Ecountry'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_003_fullwidth_underscore_path_segment',
    case: 'fullwidth underscore is rejected',
    result: 'Unicode lookalikes must not be accepted as ASCII underscore.',
    where: { metadata_match: { path: ['＿profile'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_004_cyrillic_homoglyph_path_segment',
    case: 'cyrillic homoglyph is rejected',
    result: 'Cyrillic р must not be accepted as ASCII p.',
    where: { metadata_match: { path: ['рrofile'], equals: 'x' } }, // first char is Cyrillic U+0440
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_005_combining_mark_path_segment',
    case: 'combining unicode mark is rejected',
    result: 'Combining marks must not pass path validation.',
    where: { metadata_match: { path: ['profi\u0301le'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_006_null_byte_path_segment',
    case: 'null byte inside path segment is rejected',
    result: 'Control characters must not pass path validation.',
    where: { metadata_match: { path: ['profile\u0000country'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_007_newline_path_segment',
    case: 'newline inside path segment is rejected',
    result: 'Newlines must not pass path validation.',
    where: { metadata_match: { path: ['profile\ncountry'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_008_leading_zero_index',
    case: 'leading zero array index is rejected',
    result: 'Only "0" is valid; "00" and "01" must not be accepted.',
    where: { metadata_match: { path: ['items', '01'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_009_float_index',
    case: 'float-like array index is rejected',
    result: 'Array index must be a simple decimal integer string.',
    where: { metadata_match: { path: ['items', '1.0'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_010_scientific_index',
    case: 'scientific notation array index is rejected',
    result: 'Array index must not accept scientific notation.',
    where: { metadata_match: { path: ['items', '1e2'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_011_plus_index',
    case: 'plus-prefixed index is rejected',
    result: 'Array index must not accept +1.',
    where: { metadata_match: { path: ['items', '+1'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_012_mongo_operator_path_segment',
    case: 'Mongo operator path segment is rejected',
    result: '$ne must not be accepted as a path token.',
    where: { metadata_match: { path: ['$ne'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_013_mongo_where_path_segment',
    case: 'Mongo $where path segment is rejected',
    result: '$where must not be accepted as a path token.',
    where: { metadata_match: { path: ['$where'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_014_jsonpath_array_syntax',
    case: 'JSONPath array syntax is rejected',
    result: 'JSONPath-like segments must not be accepted.',
    where: { metadata_match: { path: ['items[0]'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_015_recursive_descent_path',
    case: 'recursive descent syntax is rejected',
    result: 'Recursive JSONPath syntax must not be accepted.',
    where: { metadata_match: { path: ['..secret'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_016_sql_like_path_segment',
    case: 'SQL-looking path segment is rejected',
    result: 'Path values must never become SQL fragments.',
    where: { metadata_match: { path: ['x; DROP TABLE User; --'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_017_template_injection_path_segment',
    case: 'template-like path segment is rejected',
    result: 'Template-looking syntax must not be accepted as a path token.',
    where: { metadata_match: { path: ['${metadata}'], equals: 'x' } },
    expect_ids: [],
    expected_error: 'Invalid JSON path segment',
  },
  {
    id: 'json_security_027_equals_mongo_operator_object_is_literal',
    case: 'equals treats Mongo-looking object as literal JSON',
    result: 'A JSON value with $ne key is data, not a Mongo operator.',
    where: { metadata_match: { equals: { $ne: null } } },
    expected_error: 'Invalid JSON object key "$ne" for User.metadata',
  },
  {
    id: 'json_security_028_in_mongo_operator_object_is_literal',
    case: 'in treats Mongo-looking object as literal JSON',
    result: 'A JSON value with $ne key inside in-list is data, not a Mongo operator.',
    where: { metadata_in: [{ $ne: null }] },
    expected_error: 'Invalid JSON object key "$ne" for User.metadata',
  },
  {
    id: 'json_security_029_string_contains_sql_payload_literal',
    case: 'string_contains treats SQL-looking value as literal',
    result: 'SQL-looking string must not broaden the query.',
    where: { metadata_match: { string_contains: "' OR '1'='1" } },
    expect_ids: [],
  },
  {
    id: 'json_security_030_string_contains_mongo_payload_literal',
    case: 'string_contains treats Mongo-looking value as literal',
    result: 'Mongo-looking string must not broaden the query.',
    where: { metadata_match: { string_contains: '{$ne:null}' } },
    expect_ids: [],
  },
  {
    id: 'json_security_031_string_contains_bidi_literal',
    case: 'string_contains treats bidi control as literal',
    result: 'Bidi control in a search string must not affect query syntax.',
    where: { metadata_match: { string_contains: 'root\u202Etext' } },
    expect_ids: [],
  },
];
    ...
    describe('JsonFilterOptionalRoot', () => {
      optionalRootValueFilterTests.forEach(
        ({ id, case: title, where, expect_ids, expected_error, ...expected }) => {
          const testFn = adapterName in expected && expected[adapterName]?.skip ? test.skip : test;
          testFn(
            `${id} : ${title}`,
